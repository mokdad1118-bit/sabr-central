...
def render_cert_pdf(html):
    """Generate PDF from HTML using WeasyPrint when available.

    Falls back to raising a RuntimeError so callers can return a 500 message.
    """
    try:
        from weasyprint import HTML
    except Exception as exc:
        raise RuntimeError(f"WeasyPrint is not available: {exc}")
